...
use_lab6_agent = st.checkbox(
    "Enable Lab 6 tool-agent (multi-tool trace)",
    value=False,
    help="Runs deterministic Lab 6-style tools (retrieve/summarize/stats/log search) and returns a tool trace for grading evidence.",
)
    ...
    if not instruction or not user_input:
        ...
        with st.spinner("Running retrieval, warehouse grounding, and generation..."):
            try:
                ...
                tool_trace = result.get("lab6_tool_trace", [])
                if tool_trace:
                    st.subheader("Lab 6 Tool Trace")
                    st.json(tool_trace)
